	...
	<view>
		<block v-for="(item, index) in list_data" :key="index" v-if="getMinToMax(index)">
			<view class="bg-white margin-top padding radius zaiui-trends">
				...
				<!-- #ifndef APP-PLUS -->
				<view class="zaiui-video-box" v-if="item.video"><video class="video-view" :src="item.video" @error="videoErrorCallback" /></view>
				<!-- #endif -->

				<!-- #ifdef APP-PLUS -->
				<view class="zaiui-video-box" v-if="item.video"><video-render :isCover="item.cover" :isSrc="item.video" :isNum="index + 1" /></view>
				<!-- #endif -->
				...
				<view class="zaiui-img-grid-col" v-if="item.img.length > 0">
					...
					<view class="grid col-2" v-if="item.img.length == 2">
						<block v-for="(items, indexs) in item.img" :key="indexs">
							<view class="img-grid-view" @tap="imgTap(items, items, indexs)"><view class="img-grid" :style="[{ backgroundImage: 'url(' + items + ')' }]" /></view>
						</block>
					</view>
