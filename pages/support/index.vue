<template>
	<view>
			...
			<view id="okk" scroll-with-animation>
				...
				<view class="flex-column-start" v-for="(x, i) in msgList" :key="i">
					...
					<view v-if="!x.my" class="flex-row-start margin-left margin-top one-show">
						<view class="chat-img flex-row-center"><image style="height: 75rpx;width: 75rpx;" src="../../static/image/support/bot.png" mode="aspectFit"></image></view>
							...
							<view class="margin-left padding-chat flex-column-start" style="border-radius: 35rpx;background-color: #f9f9f9;">
								...
								<view class="flex-column-start" v-if="x.type == 1" style="color: #2fa39b;">
									...
									<view class="flex-row-start  padding-top-sm">
										...
										<!-- <text class="padding-left" style="color: #007AFF;">人工客服</text> -->
										<button open-type="contact" style="color: #007AFF;width: auto;height:auto;font-size: 28rpx;padding: 0;margin: 0;margin-left: 20rpx;">
											人工客服
										</button>
									</view>
								</view>
								...
								<view class="flex-column-start" v-if="x.type == 0">
									...
									<text @click="answer(index)" style="margin-top: 30rpx;color: #2fa39b;" v-for="(item, index) in x.questionList" :key="index">{{ item }}</text>
									<view class="flex-row-start  padding-top-sm">
										<text class="my-neirong-sm">没有你要的答案?</text>
										<!-- <text class="padding-left" style="color: #1396c5;">人工客服</text> -->
										<button open-type="contact" style="color: #007AFF;width: auto;height:auto;font-size: 28rpx;padding: 0;margin: 0;margin-left: 20rpx;">
											人工客服
										</button>
									</view>
		...
		<!-- 底部导航栏 -->
		<view class="flex-column-center" style="position: fixed;bottom: -123px;" :animation="animationData">
...
export default {
	...
	data() {
		return {
			...
			msgList: [{ my: false, msg: '你好我是客服机器人钢蛋,请问有什么问题可以帮助您?', type: 1, questionList: ['如何登录', '如何开具发票？', '如何添加或编辑地址'] }],
			msg: '',
			go: 0,
			srcollHeight: 0
		};
	...
	methods: {
		...
		answer(id) {
			...
			this.msgList.push({
				my: false,
				msg: '钢蛋还在学习中,没能明白您的问题,您点击下方提交反馈与问题,我们会尽快人工处理(无法回答模板)',
				type: 0,
				questionList: ['如何登录', '如何开具发票？', '如何添加或编辑地址']
			});
		},
		sendMsg() {
			// 消息为空不做任何操作
